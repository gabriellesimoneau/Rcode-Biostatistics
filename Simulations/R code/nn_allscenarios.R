...
est <- vector(mode = "list", length = 1)

for(i in 1:9)
{
  # reset estimates to NA for new scenario
  est[[1]] <- matrix(NA, nrow = Nsimul, ncol = 1 + Nboot)
  for(s in 1:Nsimul)
  {
    ...
    for(b in 1:Nboot)
    {
      ...
      est[[1]][s, b + 1] <- try(extract(res))
    }
  }
  # linux command to save results of the simulations in a CSV file
  colnames(est[[1]])[1] <- "psi10"
  name1 <- paste("nn_psi10_scenario", paste(sc[i]),".csv",sep ="")
  write.csv(est[[1]], file = name1, row.names = FALSE)
}
